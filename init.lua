...
require('lazy').setup({
          ...
          layout_config = {
            width = function(self, max_columns, max_lines)
              if max_columns < 100 then
                return max_columns
              elseif max_columns < 400 then
                return math.floor(max_columns * 0.6)
              else
                return 400
              end
            end,
            height = { 0.5, max = 18, min = 8 },
          },
        })
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map('<C-k>', vim.lsp.buf.signature_help, 'Open Signature')
  ...
  { -- Highlight, edit, and navigate code
    ...
    config = function()
      local parsers = { 'bash', 'c', 'go', 'gomod', 'dockerfile', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'vim', 'vimdoc' }
